 ...
 *          with 2 children.
 */
bool BinTree::insert(Generic* data) {
    if(data != NULL) {
        if (this->root == NULL) {
            // insert to root
            try {
 ...
 */
bool BinTree::insertHelper(Node* node, Generic* data) {
    Generic* ndata = node->getData();
	if (*data < *ndata) {     // assumes operator> exists
        // ignore right half
 ...
 * otherwise false.
 */
bool BinTree::remove(Generic& target) {
 ...
 */
bool BinTree::removeHelper(Node* node, Generic& target) {
    return false;
}
 ...
 * otherwise false.
 */
bool BinTree::retrieve(Generic& target, Generic*& container) {
 ...
 */
bool BinTree::retrieve(Node* subroot, Generic& target, Generic*& container) {
    bool found = false;
    if(subroot != NULL) {
 ...
 * @param target reference to the Generic object containing the content
 *        to search for
 ...
 */
int BinTree::getDepthHelper(Generic& target, Node* cur, int level) {
    if (cur == NULL)
        return 0;
 ...
 */
void BinTree::bstreeToArray(Generic* nodeArray[]) {
    int* index = new int(0);
 ...
 */
void BinTree::arrayToBSTree(Generic* nodeArray[]) {
 ...
 */
void BinTree::arrayToBSTreeHelper(Generic* nodeArray[], int lo, int hi) {
    if(!(lo > hi)) {
        int middle = (lo + hi) / 2;
        // process
 ...
 */
int BinTree::getArraySize(Generic* nodeArray[]) {
    int size = 0;
 ...
 */
BinTree::Node::Node(Generic* data, BinTree::Node* left, BinTree::Node* right) :
                data(data), left(left), right(right) {}

/**------------------------------------ Copy Constructor *//**
 * @brief copy constructor
 * @details make a new Generic object with the same content, copy every other
 * attributes as well
 *
 ...
 * @details delete the Generic object and clear all pointers
 */
BinTree::Node::~Node() {
    if(data != NULL)
        delete data;
...
/**------------------------------------ getData() *//**
 * @brief public getter for content Generic
 * @return pointer to the Generic object
 */
Generic* BinTree::Node::getData() const { return data; }

/**------------------------------------ operator== *//**
 * @brief equality comparison operator
 * @details compare for equality between two Generics
 *
 * @param other reference to the other Generic object to compare against
 ...
 * @details compare for inequality between two Generics. Just the opposite of
 * operator==
 *
 * @param other reference to the other Generic object to compare against
